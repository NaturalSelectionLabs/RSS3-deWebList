...
const WebsitePage: NextPage = () => {
    ...
    useEffect(() => {
        ...
        const loadRSSFeed = async (response: Response): Promise<WebsiteFeedListItem[]> => {
        ...
        const loadJSONFeed = async (response: Response): Promise<WebsiteFeedListItem[]> => {
        ...
        if (website != null) {
            buildFeedURL(website).then().catch((error) => {
                setError(error);
            });
        ...
        <Container>
            <header className="flex flex-col space-y-10 py-10">
                <div className="flex items-center gap-6">
                    <div className="flex flex-col">
                        ...
                        <div className="flex mt-10 space-x-8">
                            <div>
                                <Image src={props.website.logo ?? faviconImage} width={72} height={72} priority
                            ...
                            <div className="flex flex-col justify-between">
                                ...
                                <p className="text-lg font-medium leading-8 text-slate-700 line-clamp-1">
                                    {props.website.description ?? "This website has no description"}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <ul className="flex space-x-4">
                    <li>
                        <Link
                            href={`${props.websiteURL as string}`} target="_blank"
                            className="border bg-blue-500 text-white rounded py-2 px-4 font-bold hover:bg-blue-700">
                            <span>Open</span>
                        </Link>
                    </li>
                    <li>
                        <Link href={`${props.feedURL as string}`} target="_blank"
        ...
        <>
            {
                props.feeds.map((feed, index) => (
                    <Link href={feed.url} target="_blank" key={index} className="block hover:bg-slate-100">
